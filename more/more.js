...
(() => {
    ...
    var settings = {};
    var installed_scripts =
        Settings.getValue("ArmoredMore-InstalledScripts", []) || []; // All scripts installed though more.js
    var installed_repositories =
        Settings.getValue("ArmoredMore-InstalledRepositories", []) || []; // All repositories installed though more.js

    // Global vars
    var tablet;
    var more_tablet_window;
    var app_button;
    var active = false;
    ...
    async function installRepo(url) {
        ...
        // Save new list as setting
        Settings.setValue(
            "ArmoredMore-InstalledRepositories",
    ...
    async function getLists() {
        ...
        for (let i = 0; installed_repositories.length > i; i++) {
            ...
            apps = apps.filter((app) => {
                let app_root =
                    repo.url.replace(/\/metadata.json/g, "") +
                    `/${app.directory}`;
