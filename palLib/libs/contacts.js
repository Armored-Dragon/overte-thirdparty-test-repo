...
let contactsLib = {
	...
	getContactByUuid: (uuid, refreshContactList = false) => {
		return new Promise(async (resolve, reject) => {
			if (refreshContactList) await contactsLib.getContactList();

			const contactSingle = contactsLib.contacts.find((contact) => contact.location.node_id === helper.removeCurlyBracesFromUuid(uuid));
